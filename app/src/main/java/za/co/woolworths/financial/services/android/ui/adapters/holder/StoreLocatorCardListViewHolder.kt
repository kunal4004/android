...
class StoreLocatorCardListViewHolder(val itemBinding: StoreNearbyListCardItemBinding) : RecyclerView.ViewHolder(itemBinding.root) {
    fun setItem(storeDetails: StoreDetails, clickListener: (StoreDetails) -> Unit) {
        with(storeDetails) {
            ...
            itemBinding.root.setOnClickListener { clickListener(storeDetails) }
        }
    }
}