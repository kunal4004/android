...
class SupplyIncomeDetailFragment : CLIFragment(), View.OnClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.apply {
            val mHashIncomeDetail = getSerializable(IncreaseLimitController.INCOME_DETAILS) as? HashMap<String, String>
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            R.id.imInfo -> {
                val arrTitle = resources.getStringArray(R.array.supply_info_income_title)
                val arrDescription = resources.getStringArray(R.array.supply_info_income_desc)
                val incomeMap = LinkedHashMap<String, String>()
                var position = 0
                while (position < arrTitle.size) {
                    incomeMap[arrTitle[position]] = arrDescription[position]
                    position++
                }
