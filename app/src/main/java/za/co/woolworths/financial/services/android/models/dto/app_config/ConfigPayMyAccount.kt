...
data class ConfigPayMyAccount(
    ...
    var debitOrder: List<PMAPayByDebitOrderEntityItem>
) : Parcelable {
    ...
    fun addCardUrl(productGroupCode: String): String {
        return addCardBaseUrl
            ?.replace(
                "{{api_id}}",
                URLEncoder.encode(
                    WoolworthsApplication.getApiId()?.toLowerCase(Locale.getDefault()),
                    "UTF-8",
                ).toString(),
            )
    ...
    fun isFeatureEnabled(): Boolean? =
        Utils.isFeatureEnabled(minimumSupportedAppBuildNumber) ?: false
}
