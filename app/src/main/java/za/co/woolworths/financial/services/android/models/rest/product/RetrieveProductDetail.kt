...
class RetrieveProductDetail(private val productRequest: ProductRequest?, private var responseDelegate: AsyncAPIResponse.ResponseDelegate<ProductDetailResponse>?) : HttpAsyncTask<String, String, ProductDetailResponse>() {

    private var mException: String? = null

    override fun httpDoInBackground(vararg params: String?): ProductDetailResponse? {
        return WoolworthsApplication.getInstance()?.api?.productDetail(productRequest?.productId, productRequest?.skuId)
    }
