    ...
    VtoSelectOptionListener, WMaterialShowcaseView.IWalkthroughActionListener, VtoTryAgainListener,View.OnTouchListener,ReviewThumbnailAdapter.ThumbnailClickListener, ViewTreeObserver.OnScrollChangedListener {
        ...
        private fun updateReportLikeStatus() {
            ...
            private fun savePhoto(bitmap: Bitmap) {
                ...
                vtoSavedPhotoToast.showMessage(
                    requireActivity(),
                    getString(R.string.saved_to_photos),
                    250
                )
            }
        ...
        private fun openCart() {
                    ...
                    lifecycleScope.launch {
                        ...
                        try {
                            ...
                            if (response.httpCode == 200) {
                                iv_like.setImageResource(R.drawable.iv_like_selected)
                                RatingAndReviewUtil.likedReviews.add(
                                    ratingReviewResponse?.reviews?.get(
                                        0
                                    )?.id.toString()
                                )
                            }
            ...
            private fun configureDefaultUI() {
                ...
                if (mFetchFromJson) {
                    val productDetails =
                        Utils.stringToJson(activity, defaultProductResponse)!!.product
            ...
            override fun onProductDetailsSuccess(productDetails: ProductDetails) {
                ...
                otherSKUsByGroupKey =
                    this.productDetails?.otherSkus?.let { groupOtherSKUsByColor(it) }!!
            ...
            private fun showColors() {
                ...
                otherSKUsByGroupKey.size.let {
                    if (it > spanCount) {
                        ...
                        moreColor?.text = requireContext().getString(
                    ...
                    if (productDetails?.promotionsList?.isEmpty() == false) {
                        productDetails?.promotionsList?.forEachIndexed { i, it ->
                            ...
                            if (it.promotionalText?.contains(":") == true) {
                                ...
                                editedPromotionalText =
                                    "<b>" + beforeColon + ":" + "</b>" + afterColon
                            }
                            when (i) {
                                0 -> {
            ...
            override fun updateDeliveryLocation() {
                activity?.apply {
                    when (SessionUtilities.getInstance().isUserAuthenticated) {
                        ...
                        false -> ScreenManager.presentSSOSignin(
                            this,
                            SSO_REQUEST_FOR_SUBURB_CHANGE_STOCK
                        )
                    }
                    ...
                    images.forEach { image ->
                        when (image) {
                            promoImages?.reduced -> {
                                ...
                                layoutInflater.inflate(R.layout.promotional_image, null)
                                    ?.let { view ->
            ...
            override fun shareProduct() {
                activity?.apply {
                    ...
                    arguments[FirebaseManagerAnalyticsProperties.PropertyNames.ITEM_ID] =
                        productDetails?.productId
                            ?: ""
                    arguments[FirebaseManagerAnalyticsProperties.PropertyNames.CONTENT_TYPE] =
                        message
            ...
            private val pickPhotoFromFile =
                registerForActivityResult(PickImageFileContract()) { uri ->
                    uri?.let {
                        getPickedImageFile.launch(uri)
                    }
                }
            private val getPickedImageFile =
                registerForActivityResult(ImageResultContract()) { uri ->
                    uri?.let {
                        checkUriType(uri)
                    }
                }
