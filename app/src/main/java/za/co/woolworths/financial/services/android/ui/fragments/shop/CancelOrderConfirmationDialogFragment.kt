...
class CancelOrderConfirmationDialogFragment : WBottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        cancel?.apply {
            (activity as? OrderDetailsActivity)?.triggerFirebaseEvent(FirebaseManagerAnalyticsProperties.PropertyNames.CANCEL_CANCEL)
            paintFlags = Paint.UNDERLINE_TEXT_FLAG
            setOnClickListener { dismissAllowingStateLoss() }
        }
        confirmCancelOrder.setOnClickListener {
            (activity as? OrderDetailsActivity)?.triggerFirebaseEvent(FirebaseManagerAnalyticsProperties.PropertyNames.CONFIRM_CANCEL)
