...
class ItemsListToRemoveFromCartAdapter(var commerceItems: ArrayList<CommerceItem>) :
    RecyclerView.Adapter<ItemsListToRemoveFromCartAdapter.ViewHolder>() {
            ...
            itemBinding.apply {
                ...
                if (commerceItem.priceInfo.discountedAmount > 0) {
                    promotionalText.text =
                        " " + CurrencyFormatter.formatAmountToRandAndCentWithSpace(commerceItem.priceInfo.discountedAmount)
                ...
                if (commerceItem.commerceItemClassType == "foodCommerceItem") {
                    ...
                    commerceItem.commerceItemInfo?.apply {
                        if (sizeAndColor.isEmpty() && size.isNotEmpty() && !size.equals("NO SZ",
                                true)
                        )
                            sizeAndColor = size
                        else if (sizeAndColor.isNotEmpty() && size.isNotEmpty() && !size.equals("NO SZ",
                                true)
                        ) {
