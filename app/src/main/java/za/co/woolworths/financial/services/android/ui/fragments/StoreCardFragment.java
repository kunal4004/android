...
public class StoreCardFragment extends BaseAccountFragment {
    private static final String TAG = "StoreCardFragment";
    private int mStoreCardId;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.store_card_fragment, null);
        mError = WErrorDialog.getSimplyErrorDialog(getActivity());
        view.findViewById(R.id.store_card_transaction_action).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), TransactionHistoryActivity.class);
                intent.putExtra(TransactionHistoryActivity.ACCOUNT_TYPE, TransactionHistoryActivity.Accounts.STORE_CARD);
                intent.putExtra(TransactionHistoryActivity.PRODUCT_ID, mStoreCardId);
                startActivity(intent);
                getActivity().overridePendingTransition(R.anim.slide_up_in, R.anim.slide_up_out);
            }
        });
        view.findViewById(R.id.buttonLayoutStore).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), TransactionHistoryActivity.class);
                intent.putExtra(TransactionHistoryActivity.ACCOUNT_TYPE, TransactionHistoryActivity.Accounts.STORE_CARD);
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        Account account = new Gson().fromJson(getApplication().getUserManager().getAccount(UserManager.STORE_CARD), Account.class);
    ...
    private void handleAccountResponse(Account account) {
        mStoreCardId = account.productOfferingId;
        View view = getView();
        if (view == null){
            return;
        }
        WBalanceView balanceView = (WBalanceView) view.findViewById(R.id.store_card_balance);
...
//        }
        ((TextView) view.findViewById(R.id.store_card_credit_limit)).setText(FontHyperTextParser.getSpannable(WFormatter.formatAmount(account.creditLimit), 1, getActivity()));
        ((TextView) view.findViewById(R.id.store_card_available_funds)).setText(FontHyperTextParser.getSpannable(WFormatter.formatAmount(account.availableFunds), 1, getActivity()));
        ((TextView) view.findViewById(R.id.store_card_minimum_payment)).setText(FontHyperTextParser.getSpannable(WFormatter.formatAmount(account.minimumAmountDue), 1, getActivity()));
        try {
            ((TextView) view.findViewById(R.id.store_card_payment_date)).setText(FontHyperTextParser.getSpannable(WFormatter.formatDate(account.paymentDueDate), 1, getActivity()));
        } catch (ParseException e) {
            ((TextView) view.findViewById(R.id.store_card_payment_date)).setText(account.paymentDueDate);
            WiGroupLogger.e(getActivity(), TAG, e.getMessage(), e);
        }
        view.findViewById(R.id.store_card_loading).setVisibility(View.GONE);
    }

    private void handleAccountResponse(AccountResponse accountResponse) {
        switch (accountResponse.httpCode) {
            case 200:
                getApplication().getUserManager().setAccount(UserManager.STORE_CARD, accountResponse.account);
